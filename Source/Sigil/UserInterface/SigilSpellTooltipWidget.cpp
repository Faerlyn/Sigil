...
#include "../Actor/SigilSpellCaptureActor.h"
...
FText USigilSpellTooltipWidget::Get_TextBlock_Damage_Text()
{
	if (SpellRef)
	{
		...
		for (size_t i = 0; i < SpellRef->SpellDamage.Num(); i++)
		{
			DamageString.Append(UKismetTextLibrary::Conv_FloatToText(SpellRef->SpellDamage[i].DamageAmount, ERoundingMode::HalfToEven).ToString());
			DamageString.Append(" (");
			DamageString.Append(UEnum::GetDisplayValueAsText(SpellRef->SpellDamage[i].ElementType).ToString());
			DamageString.Append(")\n");
		}
